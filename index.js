...
async function run(){
    try{
        const questionsCollection = client.db('Quiz').collection('Questions');
        ...
        app.get('/questions/:name', async (req, res) => {
            ...
            const service = await questionsCollection.find(query).toArray();
            res.send(service);
        });
...
app.listen(port,()=>console.log(`Quiz Server is running on ${port}`))
